...
pub fn execute(args: ImageTessellateArgs) -> Result<()> {
    ...
    // Process files
    process(input, n_vertical, n_horizontal, output, delete_original).with_context(|| "Processing failed")?;
...
fn process(input: &Path, n_vertical: u32, n_horizontal: u32, output: &Path, delete_original: bool) -> Result<()> {
...
fn process_file(input: &Path,  n_vertical: u32, n_horizontal: u32, output: &Path) -> Result<()>{
    ...
    horizontal_coordinates.iter_mut().enumerate().take(n_horizontal as usize)
        .for_each(|(i, coord)| {
            *coord = [i as u32 * patch_width, (i as u32 + 1) * patch_width];
        });

    vertical_coordinates.iter_mut().enumerate().take(n_vertical as usize)
        .for_each(|(i, coord)| {
    ...
    for i in 0..patch_width_left_over as usize {
        ...
        horizontal_coordinates.iter_mut()
            .skip(i + 1)
    ...
    let stem = input.file_stem().with_context(|| "Can't extract stem")?.to_str().with_context(|| "Can't convert to string")?;
    let ext = input.extension().with_context(|| "Can't extract extension")?.to_str().with_context(|| "Can't convert to string")?;
    ...
    for (index, (vertical_slice, horizontal_slice)) in vertical_coordinates.iter().flat_map(|v| horizontal_coordinates.iter().map(move |h| (v, h))).enumerate() {
        ...
        // Save image
        patch.save(&output_path).with_context(|| format!("Couldn't save image to {:?}", output_path))?;
    }
