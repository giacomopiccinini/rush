...
pub fn execute(args: ImageDuplicatesArgs) -> Result<()> {
    ...
    let image_files: Vec<PathBuf> = WalkDir::new(target)
        .max_depth(1)
        .into_iter()
        .filter_map(|e| e.ok())
    ...
    // Calculate hashes
    let hashes: Vec<(String, PathBuf)> = image_files
        .par_iter()
        .filter_map(|file| process_image(file).ok().map(|hash| (hash, file.clone())))
        .collect();
